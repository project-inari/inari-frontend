<template>
  <div class="inventory-manager-page" :class="fontDMSansPrompt">
    <!-- HEADER -->
    ...
    <div class="header-actions">
      ...
      <div class="search-bar">
        <label>Search:</label>
        <PrimeInputText v-model="searchKeyword" placeholder="Type to search any field..." class="global-search-input" />
      </div>
    ...
    <PrimeDataTable v-model:selection="selectedRowItem" :value="filteredProducts" removable-sort scrollable scroll-height="400px"
      ...
      <PrimeColumn header="Stock Qty" sortable>
        <template #body="slotProps">
      ...
      <PrimeColumn>
        <template #body="slotProps">
          <PrimeButton icon="pi pi-pencil" outlined severity="secondary" @click="editItem(slotProps.data)" />
        </template>
      </PrimeColumn>
    </PrimeDataTable>
    ...
    <section v-if="selectedItem" class="dynamic-display-section">
      ...
      <div class="dynamic-display-card">
        <NuxtImg :src="selectedItem.img" alt="selected-item" class="dynamic-item-img" width="80" height="80" />
        <div class="dynamic-display-left-side">
          ...
          <div class="info-row">
            ...
            <span>{{ getCategoryPath(selectedItem.categoryId).join(' → ') }}</span>
          </div>
          <div class="info-row">
          ...
          <div class="info-row">
            <label class="dynamic-display-info-label">Purchase Price:</label>
        ...
        <div class="dynamic-display-right-side">
          ...
          <div class="dynamic-display-warehouse-info">
            <div v-for="(warehouse, index) in selectedItem.qtyInWarehouse || []" :key="index" class="stock-card">
              <div class="stock-info">
                ...
                <div class="line">
                  <span>Value:</span>
                  <strong>{{ warehouse.qty * selectedItem.purchasePrice || 0 }}</strong>
                </div>
              </div>
              <div class="divider"></div>
              <div class="warehouse-name">Warehouse {{ warehouse.warehouseId }}</div>
            </div>
          </div>
        </div>
      </div>
...
const groupedFilterOptions = ref([
  {
    label: 'Category',
    items: categories.map((cat: any) => ({ label: cat.name, value: cat.name })),
  },
  {
    label: 'Tag',
...
const selectedItem = ref<InventoryItem | null>(inventoryData.value.length ? inventoryData.value[0] : null)
...
async function viewItem(rowData: any) {
  selectedItem.value = await $fetch<InventoryItem>(`/api/business/${currentBusinessStore.businessId}/inventory/${rowData.id}`, {
    method: 'GET',
    headers: { 'Content-Type': 'application/json' },
  })
...
function getCategoryPath(categoryId: number): string[] {
  ...
  function buildPath(c: any) {
    if (c.parentCategoryId) {
      const parent = categories.find((p: any) => p.id === c.parentCategoryId)
      if (parent) {
        buildPath(parent)
      }
    }
