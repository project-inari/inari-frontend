<template>
    <div class="fulfillment-history-page" :class="fontDMSansPrompt">
        <!-- HEADER -->
        <header class="page-header">
            ...
            <div class="header-right">
                <PrimeButton label="Create Order" icon="pi pi-plus" @click="openCreateOrder" />
            </div>
        </header>
        ...
        <div class="header-actions">
            <div class="search-bar">
                <label>Search:</label>
                <PrimeInputText v-model="searchKeyword" placeholder="Search by Order ID..."
                    class="global-search-input" />
            </div>
        </div>

        <!-- FULFILLMENT TABLE (non‑selectable) -->
        <PrimeDataTable :value="filteredOrders" removable-sort scrollable scroll-height="300px"
            responsive-layout="scroll" class="order-table">
            ...
            <PrimeColumn header="Date">
                <template #body="p">{{ new Date(p.data.orderedAt).toLocaleDateString() }}</template>
            </PrimeColumn>
            <PrimeColumn header="Order Status">
                <template #body="p">{{ getStatusName(p.data.statusId) }}</template>
            </PrimeColumn>
            <PrimeColumn header="Customer">
                <template #body="p">{{ getCustomerName(p.data.customerId) }}</template>
            </PrimeColumn>
            <PrimeColumn header="Shipping Method" field="shippingMethod" />
            <PrimeColumn header="Total Price">
                <template #body="p">{{ calculateOrderTotal(p.data) }} Baht</template>
            </PrimeColumn>
        </PrimeDataTable>
...
const orders = ref<CustomerOrder[]>(
    await $fetch<CustomerOrder[]>(`/api/business/${currentBusinessStore.businessId}/order/list`)
)
const customers = ref<Customer[]>(
    await $fetch<Customer[]>(`/api/business/${currentBusinessStore.businessId}/order/customer/list`)
)
const statuses = ref<{ id: number; name: string }[]>(
    await $fetch<{ id: number; name: string }[]>(`/api/business/${currentBusinessStore.businessId}/order/status/list`)
)
