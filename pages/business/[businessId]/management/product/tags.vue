    ...
    <div class="tag-management-page" :class="fontDMSansPrompt">
        ...
        <div class="tag-content">
            ...
            <aside class="sidebar-tags">
                <h2>All Products</h2>
                <ul class="tag-list">
                    <li v-for="(tg, idx) in tagList" :key="idx"
                        :class="['tag-list-item', { active: selectedTag && selectedTag.id === tg.id }]"
                        @click="selectTag(tg)">
                        {{ tg.name }}
                    </li>
                </ul>
...
const tagList = await $fetch(`/api/business/${currentBusinessStore.businessId}/tag/list`, {
    method: 'GET',
...
// 5) Inventory Data
const inventoryList = await $fetch(`/api/business/${currentBusinessStore.businessId}/inventory/list`, {
    method: 'GET',
    headers: { 'Content-Type': 'application/json' },
})
...
const filteredItems = computed(() => {
    ...
    return inventoryData.value.filter(item =>
        item.tags && item.tags.some((t: any) => t.id === selectedTag.value.id)
    )
})
...
function calculateTotalStockQty(item: any) {
    ...
    return item.qtyInWarehouse.reduce((sum: number, wh: any) => sum + wh.qty, 0)
}
</script>
