<template>
    <div>
        ...
        <PrimeDialog v-model:visible="visible" maximizable modal header="Stock Movement" :style="{ width: '70rem' }"
            :class="fontDMSansPrompt">
            <!-- Movement Info Fields -->
            <div class="movement-form">
                <div class="movement-field">
                    <label>Reason:</label>
                    <PrimeInputText v-model="movementData.reason" placeholder="Enter reason" />
                </div>
                <div class="movement-field">
                    <label>From Warehouse:</label>
                    <PrimeDropdown v-model="movementData.fromWarehouseId" :options="warehouseOptions"
                        option-label="name" option-value="id" placeholder="Select Source Warehouse" />
                </div>
                <div class="movement-field">
                    <label>To Warehouse:</label>
                    <PrimeDropdown v-model="movementData.toWarehouseId" :options="warehouseOptions" option-label="name"
                        option-value="id" placeholder="Select Destination Warehouse" />
                </div>
                <div class="movement-field date-time-row">
                    <div class="date-field">
                        <label>Date:</label>
                        <PrimeCalendar v-model="movementData.movementDate" dateFormat="yy-mm-dd"
                            placeholder="Select date" />
                    </div>
                    <div class="time-field">
                        <label>Time:</label>
                        <PrimeCalendar v-model="movementData.movementTime" timeOnly hourFormat="24"
                            placeholder="Select time" />
                    </div>
                </div>
                <div class="movement-field">
                    <label>Note:</label>
                    <PrimeInputText v-model="movementData.note" placeholder="Additional notes..." />
                </div>
            </div>
            ...
            <PrimeDataTable :value="movementItems" removable-sort scrollable scroll-height="300px"
                ...
                <PrimeColumn header="Stock Values">
                    <template #body="slotProps">
                        <span>{{ slotProps.data.purchasePrice * calculateTotalStockQty(slotProps.data) }}</span>
                    </template>
                </PrimeColumn>
                <PrimeColumn field="moveQty" header="To Move Qty" sortable>
                    <template #body="slotProps">
                        <PrimeInputNumber v-model="slotProps.data.moveQty" show-buttons :min="0" fluid />
                    </template>
                </PrimeColumn>
            ...
            <div class="footer-actions">
                <PrimeButton label="Cancel" icon="pi pi-times" class="p-button-text p-button-danger"
                    @click="onCancel" />
                <PrimeButton label="Move" icon="pi pi-check" @click="onSubmitStockMovement" />
            </div>
        </PrimeDialog>

        <!-- Add Item Dialog -->
        <PrimeDialog v-model:visible="isAddItemModalVisible" modal header="Select Item from Warehouse"
            :class="fontDMSansPrompt">
...
function onSubmitStockMovement() {
    ...
    const payload = {
        ...
        items: movementItems.value.map(i => ({ itemId: i.id, sku: i.sku, quantity: calculateTotalStockQty(i) }))
    }
