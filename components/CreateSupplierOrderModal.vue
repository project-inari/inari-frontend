        ...
        >
            ...
            >
                ...
                <div class="group-inputs">
                    ...
                    />
                    <PrimeInputText
                        v-model="groupInputs[supplierKey].shippingMethod"
                ...
                >
                    ...
                    <PrimeColumn header="Stock Qty">
                        <template #body="slotProps">
                            <span>{{
                                aggreateStockInAllWarehouses(slotProps.data)
                            }}</span>
                        </template>
                    </PrimeColumn>
    ...
    for (const supplierKey of Object.keys(groupedItems.value)) {
        if (!supplierInfoMap.value[supplierKey]) {
            const res = await $fetch(
                `/api/business/${currentBusinessStore.businessId}/supplier/inquiry`,
                {
...
const grandTotal = computed(() => {
    return Object.entries(groupedItems.value).reduce(
        (sum, [supplierKey, items]) => {
            const itemsTotal = items.reduce(
                (subtotal, item) =>
                    subtotal + item.purchasePrice * (item.orderQty ?? 0),
                0,
            );
            const shipping = groupInputs[supplierKey]?.shippingCost ?? 0;
